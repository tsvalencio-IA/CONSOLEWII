// =============================================================================
// SUPER BOXING STADIUM: CHAMPIONSHIP EDITION (ULTIMATE MOBILE FIX)
// ARQUITETO: SENIOR DEV V100 - RESPONSIVE UI & RAW INPUT
// =============================================================================
...
(function() {
    ...
    const CONF = {
        ROUNDS: 3,
        ROUND_TIME: 60,
        PUNCH_THRESH: 12,    // Sensibilidade do soco (menor = mais fácil)
        BLOCK_DIST: 110,     // Distância para defesa
        SMOOTHING: 0.7       // 0.1 = muito lag, 1.0 = instantâneo (Aumentado para fidelidade)
    };

    // Utils
    const Utils = {
        dist: (x1, y1, x2, y2) => Math.hypot(x2 - x1, y2 - y1),
        // Mapeamento de tela corrigido para mobile (Centraliza e escala)
        toScreen: (kp, w, h) => ({ 
    ...
    const Game = {
        ...
        // UI Layout System (Armazena posições dos botões para clique preciso)
        uiButtons: {}, 
        ...
        // --- SISTEMA DE INPUT REVISADO (TOUCH PRECISO) ---
        setupInput: function() {
            window.System.canvas.onclick = (e) => {
                ...
                // Helper para verificar clique nos botões desenhados
                const checkBtn = (name) => {
        ...
        update: function(ctx, w, h, pose) {
            this.frame++;
            this.uiButtons = {}; // Limpa botões a cada frame para recalcular layout responsivo
        ...
        // --- INPUT: MELHORADO PARA SER MAIS FIEL ---
        processInput: function(w, h, pose) {
            if (!pose || !pose.keypoints) return;
            
            const k = pose.keypoints;
            // Detectar pontos (com threshold baixo para não falhar)
        ...
        updateHand: function(hand, target, side, w, h) {
            ...
            // Move a mão visualmente (quase 1:1)
            hand.x = target.x;
            hand.y = target.y;

            // Detecta Soco: Alta velocidade + Mão longe do corpo (Y) ou se movendo para cima rápido
            // Simplificado: Se velocidade > X e Stamina ok
            if (velocity > CONF.PUNCH_THRESH && hand.state === 'IDLE' && this.p1.stamina > 10) {
                // Filtro extra: movimento deve ser "para frente" (ou agressivo no eixo Y/X)
                hand.state = 'PUNCH';
                hand.z = 0; 
                this.p1.stamina -= 12;
        ...
        updateAI: function(w, h) {
            ...
            // AI Movimento mais suave que o player
        ...
        // -----------------------------------------------------------------
        // UI & MENUS (RESPONSIVO E CALCULADO)
        // -----------------------------------------------------------------
        
        // Helper para desenhar botão e salvar área de clique
        drawBtn: function(ctx, key, txt, x, y, w, h, active=false) {
            // Salva área para o input handler
            ...
            // Round Rect manual para compatibilidade
            ctx.beginPath();
            ctx.rect(x, y, w, h);
            ctx.fill();
            ...
            ctx.shadowBlur = 0;
            ctx.fillStyle = "#fff";
            ctx.font = `bold ${Math.floor(h*0.4)}px 'Russo One'`;
        ...
        uiModeSelect: function(ctx, w, h) {
            // Unidade relativa para escalar tudo proporcionalmente
            const rem = Math.min(w, h) / 10; 
            
            ctx.fillStyle = "#fff"; 
            ctx.font = `bold ${rem * 1.5}px 'Russo One'`; 
            ctx.textAlign = "center";
            ctx.fillText("SUPER BOXING", w/2, h * 0.2);
            
            const btnW = Math.min(400, w * 0.8);
            const btnH = rem * 1.5;
        ...
        uiCharSelect: function(ctx, w, h) {
            ...
            ctx.fillStyle = "#fff"; ctx.font = `bold ${rem*1.2}px 'Russo One'`;
            ctx.fillText(c.name, w/2, h*0.6);
            
            // Botões de Navegação
            const btnW = Math.min(200, w * 0.4);
            this.drawBtn(ctx, 'btnNextChar', "TROCAR", w/2 - btnW - 10, h*0.75, btnW, rem);
            this.drawBtn(ctx, 'btnConfirm', "CONFIRMAR", w/2 + 10, h*0.75, btnW, rem, true);
        },
        ...
        uiArenaSelect: function(ctx, w, h) {
            ...
            ctx.font = `bold ${rem*1.2}px 'Russo One'`;
            ctx.fillText(a.name, w/2, h*0.5);
            
            const btnW = Math.min(200, w * 0.4);
            this.drawBtn(ctx, 'btnNextArena', "TROCAR", w/2 - btnW - 10, h*0.75, btnW, rem);
            this.drawBtn(ctx, 'btnFight', "LUTAR!", w/2 + 10, h*0.75, btnW, rem, true);
        },
        ...
        uiLobby: function(ctx, w, h) {
            ...
            ctx.font = `bold ${rem}px 'Russo One'`;
            ...
            ctx.beginPath(); ctx.arc(0,0,rem,0, 5); ctx.stroke();
            ctx.restore();
        },
        ...
        uiGameOver: function(ctx, w, h) {
            ...
            this.drawBtn(ctx, 'btnMenu', "MENU PRINCIPAL", w/2 - btnW/2, h*0.7, btnW, rem*1.5, true);
        },
        ...
        drawGlove: function(ctx, hand, color, isSelf) {
            ...
            if (isSelf) { s = 1.5 + (hand.z * 0.02); y += (100 - hand.z); } 
            else { s = 0.8 + (hand.z * 0.015); y += (hand.z * 1.5); }
            ctx.save(); ctx.translate(x, y); ctx.scale(s, s);
            if (hand.state === 'PUNCH') { ctx.shadowColor = color; ctx.shadowBlur = 20; }
            ...
            ctx.strokeStyle = 'rgba(0,0,0,0.3)'; ctx.lineWidth=3; ctx.beginPath(); ctx.arc(0,0,40,0,Math.PI*2); ctx.stroke();
            ctx.restore();
        },
        ...
        drawHUD: function(ctx, w, h) {
            ...
            ctx.fillText(CHARACTERS[this.p1.charId].name, 20, 20 + barH + 20);
            ...
            ctx.fillText(this.isOnline ? "RIVAL" : "CPU", w - 20, 20 + barH + 20);

            ctx.fillStyle = "#fff"; ctx.textAlign="center"; ctx.font="bold 40px 'Russo One'";
            ctx.fillText(Math.ceil(this.timer/60), w/2, 50);
        },
        ...
        drawMsgs: function(ctx) {
            this.msgs.forEach(m => { m.y-=1; m.life--; ctx.fillStyle=m.c; ctx.font="bold 40px 'Russo One'"; ctx.textAlign="center"; ctx.fillText(m.t, m.x, m.y); });
        ...
        endRound: function() {
            ...
            else { this.state = 'GAMEOVER'; window.System.msg("TIME OVER"); }
        }
    };
